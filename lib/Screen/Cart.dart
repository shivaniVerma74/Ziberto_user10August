...
import 'package:eshop_multivendor/Screen/teeeeee.dart';
...
class StateCart extends State<Cart> with TickerProviderStateMixin {
  ...
  Widget cartItem(int index, List<SectionModel> cartList) {
    ...
    cartList[index].perItemTotal =
        (price * double.parse(cartList[index].qty!)).toString();
   packingCharges = cartList[index].productList![0].packing_charge.toString() == ""
        ? "0.0"
        : cartList[index].productList![0].packing_charge.toString();
    itemTotal = (double.parse(cartList[index].perItemTotal.toString()) +
        double.parse(packingCharges!)).toString();
    // gstAmount = (double.parse(itemTotal) * double.parse(GST!)/100).toString();
    //     gstTotal= (double.parse(itemTotal.toString())+ double.parse(gstAmount.toString())).toString();
    // print(gstAmount);
    // print(gstTotal);
    ...
    if (_isNetworkAvail) {
      try {
        ...
        if (!error) {
          ...
          print(data[0]["product_details"][0]["sellerlatitude"]);
           print(data[0]["product_details"][0]["logitude"]);
          // print(data["longitude"]);
          if (data[0]["product_details"][0]["sellerlatitude"] != null && data[0]["product_details"][0]["logitude"] != null) {
            itemLat = double.parse(data[0]["product_details"][0]["sellerlatitude"].toString());
            itemLong = double.parse(data[0]["product_details"][0]["logitude"].toString());
            if (itemLong != 0.0 && itemLat != 0.0) {
              final double km = distance.as(
                  LengthUnit.Kilometer,
                  new LatLng(itemLat, itemLong),
                  new LatLng(double.parse(loc.lat), double.parse(loc.lng)));
              deliveryChargeApi(km);
            }
          }
                      ...
                      SimBtn(
                          ...
                          onBtnSelected: () async {
                            if (oriPrice > 0) {
                              ...
                              if (isAvailable) {
                                gstAmount = (double.parse(oriPrice.toString()) * double.parse(GST!)/100) ;
                                print(gstAmount);
                                print(promoAmt);
                                totalPrice = (oriPrice- promoAmt) + delCharge + gstAmount ;
                                print(totalPrice);
                                checkout(cartList);
                              } else {
                                setSnackbar(
  ...
  doPayment() {
    print("del charge again ${delCharge.toString()} and ${oriPrice.toString()} and ${totalPrice.toString()}");
  ...
  Future<void> placeOrder(String? tranId) async {
    // totalPrice = oriPrice + delCharge + double.parse(packingCharges!);
   // setState((){
   //   totalPrice = delCharge + oriPrice + packingCharges;
   // });
    ...
    if (_isNetworkAvail) {
      ...
      try {
        var parameter = {
          USER_ID: CUR_USERID,
          PACKIMG_CHARGE: packingCharges,
          MOBILE: mob,
          PRODUCT_VARIENT_ID: varientId,
          QUANTITY: quantity,
          TOTAL: oriPrice.toStringAsFixed(2),
          FINAL_TOTAL: totalPrice.toStringAsFixed(2),
          DEL_CHARGE: delCharge.toStringAsFixed(2),
          TAX_AMT: gstAmount.toStringAsFixed(2),
          "gst": GST.toString(),
            ...
            children: [
              ...
              Row(
                ...
                children: [
                  ...
                  Text(
                    CUR_CURRENCY! + " " + gstAmount.toStringAsFixed(2),
                    style: TextStyle(
                        color: Theme.of(context).colorScheme.fontColor,
                        fontWeight: FontWeight.bold),
                  )
  ...
  void confirmDialog() {
    print("del charge ${delCharge.toString()}");
    showGeneralDialog(
  ...
  Future<void> deliveryChargeApi(km) async {
    _isNetworkAvail = await isNetworkAvailable();
    if (_isNetworkAvail) {
      try {
        context.read<CartProvider>().setProgress(true);
